...
let datosEnergiaSolar = {}; // Objeto para almacenar los datos del CSV
...
//  Procesar los datos del CSV
                    results.data.forEach(row => {
                        const year = row.Year;
                        const porcentaje = row.SolarPercentage;
                        if (year && porcentaje) {
                            datosEnergiaSolar[year] = porcentaje;
                        }
                    });
                }
            });
...
document.getElementById("solarForm").addEventListener("submit", function(event) {
    ...
    if (isNaN(year) || !(year in datosEnergiaSolar)) {
        year = Math.max(...Object.keys(datosEnergiaSolar).map(Number));
    }

    // Calcular el porcentaje de energía solar
    const porcentajeSolar = datosEnergiaSolar[year] || 0; // Evitar errores si el año no está
